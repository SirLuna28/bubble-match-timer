...
interface RewardChoiceModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelectReward: (reward: 'extraTime' | 'timeSlow' | 'stickingBubble' | 'bombBubble' | 'replay') => void;
  isLoading?: boolean;
  isReplayMode?: boolean;
}

export function RewardChoiceModal({ isOpen, onClose, onSelectReward, isLoading = false, isReplayMode = false }: RewardChoiceModalProps) {
  const [selectedReward, setSelectedReward] = useState<string | null>(null);

  const handleSelectReward = (reward: 'extraTime' | 'timeSlow' | 'stickingBubble' | 'bombBubble' | 'replay') => {
      ...
      <DialogContent className="max-w-md bg-slate-900 border-cyan-500/50">
        <DialogHeader>
          <DialogTitle className="text-2xl text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-magenta-500">
            {isReplayMode ? '🎮 Replay Level' : '🎁 Choose Your Reward!'}
          </DialogTitle>
          <DialogDescription className="text-cyan-300">
            {isReplayMode ? 'Watch an ad to replay this level' : 'Watch an ad to earn one of these amazing power-ups'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3 py-4">
          {isReplayMode ? (
            /* Replay Level Option */
            <button
              onClick={() => handleSelectReward('replay')}
              disabled={isLoading || selectedReward !== null}
              className="w-full p-4 rounded-lg border-2 border-purple-500/50 bg-slate-800 hover:bg-slate-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed text-left"
            >
              <div className="flex items-center gap-3">
                <span className="text-3xl">🎮</span>
                <div>
                  <p className="font-bold text-purple-400">Replay This Level</p>
                  <p className="text-sm text-gray-300">Start over with a fresh attempt</p>
                </div>
              </div>
            </button>
          ) : (
